billing_account                 = "billingAccounts/011A35-FD2527-D53035"                 # MUST CHANGE: Update after "/" with Billing ID associated with the GCP account
compliance_regime               = "FEDRAMP_HIGH"                                         # Already set to FedRAMP High. Compliance regime for assured workloads
assured_workload_name           = "aswl23-fldr-fedramph"                                 # If you want to recognize the folder you are working on, change the title of this Assured Workload Folder Name to be created
project_number                  = "304179763222"                                         # MUST CHANGE: Update wityh Assured workload project number you created in the Solution Guide
project_id                      = "assured-fedramp"                                      # MUST CHANGE: Update with yourt Assured Workload Project Id you created in the Solution Guide
cmek_project_id                 = "aswl23-fedramp-cmek-proj"                             # MUST CHANGE: Provide the cmek project id for this variable which will create the CMEK project where the cmek keys are stored.
location                        = "us"                                                   # Assured Worload location
organization_id                 = "468180499708"                                         # MUST CHANGE: Provide the GCP account org id that you created the project in. 
...
owner_email                     = "dataadmins@assuredworkloadsfortesting.com"            # MUST CHANGE. Group Email Id for BigQuery access (Data Admin group email id)
domain_name                     = "assuredworkloadsfortesting.com"                       # MUST CHANGE. Domain name - BigQuery
...
super_admin_group_email         = "superadmins@assuredworkloadsfortesting.com"           # MUST CHANGE. These are IAM groups that we have created - Super admin  group email id
admin_group_email               = "admins@assuredworkloadsfortesting.com"                # MUST CHANGE. These are IAM groups that we have created - Admin admin group email id
modeladmin_group_email          = "modeladmins@assuredworkloadsfortesting.com"           # MUST CHANGE. These are IAM groups that we have created - Model Admin group email id
dataadmin_group_email           = "dataadmins@assuredworkloadsfortesting.com"            # MUST CHANGE. These are IAM groups that we have created - Data Admin group email id
networksecadmin_group_email     = "networksecurityadmins@assuredworkloadsfortesting.com" # MUST CHANGE. These are IAM groups that we have created - Network and Security admin email id
...
local_location_of_udf_pubsub_to_bq      = "/home/admin_/FedRAMP-High-Terraform/dataflow-udf/transform.js"  # MUST CHANGE: location path of UDF function
...
gcs_admin_group_id                      = "admins@assuredworkloadsfortesting.com"                          # MUST CHANGE. The gcs admin group id that will be connected to the buckets
gcs_write_group_id                      = "admins@assuredworkloadsfortesting.com"                          # MUST CHANGE. The gcs write group id that will be connected to the buckets
...
bq_owner_group_email                = "dataadmins@assuredworkloadsfortesting.com"                    # MUST CHANGE. Group Email Id for BigQuery access (Data Admin group email id)
bq_domain_name                      = "assuredworkloadsfortesting.com"                               # MUST CHANGE. Domain name for biqquery access
pubsub_to_bq_table_id               = "pubsub-to-bq-transform-table1"                                # table name
pubsub_to_bq_table_schema_file_path = "/home/admin_/FedRAMP-High-Terraform/schema/tableschema.json"  # MUST CHANGE: path to schema
