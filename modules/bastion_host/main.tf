...
#This module will create a new service account for compute engine bastion and ensure that your VM instance is not associated with the default GCP service account
module "vm_service_account" {
  ...
  service_account_roles_list = var.roles_list
}

#This Updates the IAM policy to grant a role to a new member. The members to which roles are assigned here are either existing service account or new service account created for Compute Engine bastion
...
#This module will be used to create keyring, encryption/decryption keys for compute engine bastion and ensure that the customer encryption keys will be used for encryption of data stored on Google Compute Engine bastion disk
module "kms" {
...
#This will create IAM role binding and give encrypter/decrypter role to the compute engine bastion service account to encrypt/decrypt Google Compute Engine bastion disk
...
#This module will create an instance template which will be used to create compute engine bastion. This template enables confidential and shielded security on VM, enables/disables nested virtualization, enable/disables IP forwarding and provide other security controls
...
#This module will create the Compute Engine bastion virtual machine from instance template and also enables deletion protection functionality on the instance
